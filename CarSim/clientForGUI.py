...
if __name__ == "__main__":
    ...
    port=3001
    mode="Solo"
    C= snakeoil.Client()#(f='./default_parameters', t=trackName, s=stage,maxSteps=steps,p=port)
    ...
    for step in range(C.maxSteps,0,-1):
        try:
            ...
            C.respond_to_server()
            final_steps+=1            
            if C.S.d['trackPos']<-1 or C.S.d['trackPos']>1:
                    #print(f"C.S.d['trackPos']: {C.S.d['trackPos']}")
                if isOffTrack==False:
                    isOffTrack = True
                    startTimeOffTrack = C.S.d['curLapTime']
            else:
                if isOffTrack == True:
                    isOffTrack = False
                    stopTimeOffTrack = C.S.d['curLapTime']
                    totalTimeOffTrack += (stopTimeOffTrack - startTimeOffTrack)
